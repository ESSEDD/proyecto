...
function formularioInsertar($conexion){
    ...
    while($fila = mysqli_fetch_assoc($resultado)){
        if(strpos($fila['Field'],"_") !== false){
            ...
            $peticion2 = "SELECT Identificador, ".explode("_",$fila['Field'])[1]." AS campo FROM ".explode("_",$fila['Field'])[0].";";
            $resultado2 = mysqli_query($conexion, $peticion2);
            while($fila2 = mysqli_fetch_assoc($resultado2)){
...
function formularioActualizar($conexion){
    echo "<h4>Actualizo elemento para la tabla:".$_GET['tabla']."</h4>";
    ...
    while($fila = mysqli_fetch_assoc($resultado)){
        foreach($fila as $columna=>$campo){
            if(strpos($columna,"_") !== false){
                ...
                $peticion2 = "SELECT Identificador, ".explode("_",$columna)[1]." AS campo FROM ".explode("_",$columna)[0].";";
                $resultado2 = mysqli_query($conexion, $peticion2);
                while($fila2 = mysqli_fetch_assoc($resultado2)){
                    echo"
